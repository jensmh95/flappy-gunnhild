...
def score_display(game_state):
    ...
    if game_state == "game_over":
        beer_score_surf = game_font.render(
            f"{character_labels[selected_character_index]} ta {int(beer_score)} slurker",
            True,
            (255, 255, 255),
        )
        beer_score_rect = beer_score_surf.get_rect(center=(288, 300))
        shot_score_surf = game_font.render(
            f"og {int(shot_score)} shots",
            True,
            (255, 255, 255),
        )
...
pygame.time.set_timer(SPAWNCOIN, 1500)
coin_height = [300, 400, 500]


# Game over screen
# game_over_surface = pygame.transform.scale2x(
#     pygame.image.load("sprites/pilsd.png").convert_alpha()
# )
...
def main_menu():
    ...
    while True:
        ...
        MENU_TEXT = get_font(30).render("FLAPPY GUNNHILD", True, "#FBCC0B")
